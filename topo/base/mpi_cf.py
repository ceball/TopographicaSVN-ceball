...
class MPI_CFProjection_node(CFProjection):
    ...
    def _create_cfs(self):
        def stepped_array(start,stop,dtype):
            return 1.0+np.arange(0,stop,dtype=dtype)/stop

        weights_of = transferfn.DivisiveNormalizeL1()

        if self.rank==0:
            items_per_node = int(round(len(self.flatcfs_ref)/self.size))
        else:
            items_per_node = 0
        items_per_node = self.comm.bcast(items_per_node,root=0)


        for i,cf_position in enumerate(self.flatcfs):
            cf = self._create_cf(*cf_position)
            self.flatcfs[i] = cf
    	    # (2nd term could be outside loop)
            new_weights = items_per_node*self.rank+i+stepped_array(0,len(cf.weights.flat),cf.weights.dtype)
	    weights_of(new_weights)
	    # overwrite existing weights with new ones
            cf.weights[:] = new_weights.astype(cf.weights.dtype).reshape(cf.weights.shape)
            """
